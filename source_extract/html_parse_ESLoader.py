...
class ESLoader:
    ...
    def getPercentage(self,part, total):
        if(part ==0):
            pct = round(0.0,2)
        else:
    ...
    def parseHTML(self,htmlData, pageId):
        print("---------------Start-----------")
        ...
        for summaryIndex in range(0, currentIndex):
            ...
            if innerAnchor and innerAnchor != -1:
                ...
                summary.year = year
                print(json.dumps(summary, default=self.obj_dict))
                break
        # ANALYSTS
        analysts = []
        currentIndex = index + 1
    ...
    def check_if_exists_in_list(self,input_list, value):
    ...
    def maincall(self, dataResponse, fileName):
        ...
        summary = self.parseHTML(page, reference)
        ...
        for objectQA in summary.QAList:
            ...
            data.sentiment = self.get_sentiment_by_blob(data.question)
            data.answerSentiment = self.get_sentiment_by_blob(data.answer)
            qaJSON = json.dumps(data, default=self.obj_dict)
